...
do
	...
	mkdir -p data/bams/PPTO/"$name".trim
	qsub -l mem_free=8G -l h_rt=08:00:00 -N Align.$name.trim -q all.q -e stdout/align/ -o stdout/align/ \
	/mnt/work1/users/lupiengroup/People/qamraa99/common.scripts/atac-seq-align.v1.hg38.trim3.25bp.sh $f data/bams/PPTO/"$name".trim/ "$name".trim ;
done

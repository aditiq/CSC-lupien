...
grep -v chrM $workingdir/results/$analysisname/backgroundfile.annotated | grep -v random | grep -v chrUn  | grep -v alt | grep -v KI27 | awk -F'\t' '{ if (NR>1) print $1,$2,$3, $5}'  | awk 'BEGIN { FS="(" ; OFS="\t"} { print $1}' | awk -F' '  '{ print $1"\t"$2"\t"$3"\t"$1"."$4} ' | sed -e 's/'\''/prime/g' | awk -F"\t" '{print >  "'${workingdir}'""/results/""'${analysisname}'""/backgroundfile.anno."$4}'
grep -v chrM $workingdir/results/$analysisname/queryfile.annotated | grep -v random | grep -v chrUn  |   grep -v alt | grep -v KI27 |  awk -F'\t' '{ if (NR>1) print $1,$2,$3, $5}'  | awk 'BEGIN { FS="(" ; OFS="\t"} { print $1}' | awk -F' '  '{ print $1"\t"$2"\t"$3"\t"$1"."$4} ' | sed -e 's/'\''/prime/g' | awk -F"\t" '{print >  "'${workingdir}'""/results/""'${analysisname}'""/queryfile.anno."$4}'

...
for  (name in c("G523","G583","G567")){
    ...
    c3d <- fread(paste0("results/PCSC1/C3D/finalresults/", name,".combined.results.txt"), header=T, sep="\t", stringsAsFactors=F, data.table=F)
    ...
    write.table( freqdf.reshape, file=paste0("results/PCSC1/C3D/finalresults/GBM.Single.", name, ".Hic.overlap.1kb.freqdf.txt"), sep="\t", quote=F, row.names=F, col.names=T)
    ...
    write.table( summ2, file=paste0("results/PCSC1/C3D/finalresults/GBM.Single.", name, ".Hic.overlap.1kb.dcast.Percentage.txt"), sep="\t", quote=F, row.names=F, col.names=T)
    ...
    summ[is.na(summ)] <- 0
    summ2 <- dcast(summ, grplist~grp2list)
    write.table( summ2, file=paste0("results/PCSC1/C3D/finalresults/GBM.Single.Shuffled.", name, ".Hic.overlap.1kb.dcast.Percentage.txt"), sep="\t", quote=F, row.names=F, col.names=T)
}

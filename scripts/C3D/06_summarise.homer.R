...
motiffiles <- dir("results/PCSC1/C3D/finalresults/homer/", pattern="Motif.HeatmapData")
...
for ( f in notshared){
  tryCatch({
    ...
    print(name)
    a <- read.table(paste0("results/PCSC1/C3D/finalresults/homer/",f), sep="\t", header=T, stringsAsFactors=F,  row.names=NULL)
    colnames(a) <- gsub(paste0(name, ".FE"), "", colnames(a))
    colnames(a)[1] <- "TF"
    a$sum <- apply(a[,-1],1,sum)
    print(range(a$sum))
    tf <- subset(a$TF, a$sum>=2)
    write.table(tf, file=paste0("results/PCSC1/C3D/finalresults/homer/", "TFs.notshared.", name, ".txt"), sep="\t", row.names=F, col.names=F, quote=F)
    }, error=function(e){})
}
...
for ( f in common){
  tryCatch({
    ...
    write.table(tf, file=paste0("results/PCSC1/C3D/finalresults/homer/", "TFs.common.", name, ".txt"), sep="\t", row.names=F, col.names=F, quote=F)
    }, error=function(e){})
}
...
for ( f in common){
  tryCatch({
    ...
    a2 <- read.table(paste0("results/PCSC1/C3D/finalresults/homer/",name2), sep="\t", header=T, stringsAsFactors=F,  row.names=NULL)
    colnames(a2)[1] <- "TF"
    ...
    print(range(a$sum))
    tf <- subset(a$TF, a$sum == 3)
    write.table(tf, file=paste0("results/PCSC1/C3D/finalresults/homer/", "TFs.common.butnotinnotshared.", name, ".txt"), sep="\t", row.names=F, col.names=F, quote=F)
    }, error=function(e){})
}
...
for ( f in shared){
  tryCatch({
    ...
    a$sum <- apply(a[,-1],1,sum)
    tf <- subset(a$TF, a$sum >=2)
    write.table(tf, file=paste0("results/PCSC1/C3D/finalresults/homer/", "TFs.shared.", name, ".txt"), sep="\t", row.names=F, col.names=F, quote=F)
  }, error=function(e){})
}
...
for ( f in shared){
  tryCatch({
    ...
    print(range(a$sum))
    tf <- subset(a$TF, a$sum >=2)
    write.table(tf, file=paste0("results/PCSC1/C3D/finalresults/homer/", "TFs.shared.butnotinnotshared.", name, ".txt"), sep="\t", row.names=F, col.names=F, quote=F)
    }, error=function(e){})
}
...
tfs <- dir("results/PCSC1/C3D/finalresults/homer/", pattern="TFs.", full.names=T)
tfs <- tfs[-grep("butnotinnotshared", tfs)]


tflistall <- tfs[grep("KitchenSink1", tfs)]
tflist <- list()
...
colnames(tf.df) <- gsub("TFs.","",gsub(".Motif.HeatmapData\\\\.shared", "",
 gsub(".KitchenSink1.Consensus.Catalogue.narrowPeak.txt", "", colnames(tf.df))))
...
pdf(paste0("results/PCSC1/C3D/finalresults/homer/Heatmap.TF.KitchenSink1.pdf"))
print(p)
dev.off()

...
binarymatds="data/ConsensusSet/PCSC1/PFA.BRAIN.Consensus.Catalogue.Binarymat.txt"
...
#--------------------------------
bg <- c(rep(1,37))
cl <- c(rep(1,20), rep(2, 7), rep(1, 10))
...
## Run scabc 
#--------------------------------
pfa.brain.scabc = getClusterSpecificPvalue(ForeGround=as.matrix(ncounts), cluster_assignments =cl, background_medians = bg)
save(pfa.brain.scabc, file=paste0("results/PCSC1/Cluster/scABC/scABC.PFA.BRAIN.peakselection.Rdata"))

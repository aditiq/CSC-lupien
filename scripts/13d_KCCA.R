...
binarymat <- read.table("results/PCSC1/immune.landscape/ImmuneTSSOverlap.txt", header=T, sep="\t", stringsAsFactors = F,check.names = F)
rownames(binarymat) <- paste(binarymat[,100],binarymat[,100],sep="_")

mat <- as.matrix(binarymat[,5:99])
...
save(kcca.cl, file=paste0("results/PCSC1/Cluster/KCCA.Flexclust/kccadata/KCCA.immune.", numk,".Rdata"))

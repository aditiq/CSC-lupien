...
#----------------------------------------------------------------
set.seed(2017)
my_counts_matrix <- fread("results/PCSC1/CREAM/KitchenSink2.CREAM.Binarymat.txt", header=T, sep="\t", stringsAsFactors=F, data.table=F)
...
save(motif_ix,file="results/PCSC1/chromVAR/CREAM/KS2/motif_ix.Rdata")

#----------------------------------------------------------------
## compute deviation
#----------------------------------------------------------------
dev <- computeDeviations(object = counts_filtered, annotations = motif_ix)
save(dev,file="results/PCSC1/chromVAR/CREAM/KS2/Combined.PCSC1.extended.dev.Rdata")
z.scores = deviationScores(dev) ## deviation Z-score
dev.scores = deviations(dev) ## bias corrected deviations

write.table(z.scores, file="results/PCSC1/chromVAR/CREAM/KS2/Dev.Zscore.txt", col.names=T, row.names=T, sep="\t", quote=F)
write.table(dev.scores, file="results/PCSC1/chromVAR/CREAM/KS2/Deviations.txt", col.names=T, row.names=T, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/Variability.pdf") ;
plotVariability(variability, use_plotly = FALSE) ;
dev.off()

write.table(variability, file="results/PCSC1/chromVAR/CREAM/KS2/Variability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/VariabilityRanked.pdf", useDingbats = F) ;
...
colnames(top_devs) <- gsub("_peaks","", colnames(top_devs))

rownames(mapping) <- gsub("-includes-blacklisted-regions.hg38","", rownames(mapping))
rownames(mapping) <- gsub("_peaks","", rownames(mapping))

annocol1 <- data.frame(group=mapping$group1, stem=mapping$stem,  stringsAsFactors=F)
rownames(annocol1) <- colnames(top_devs)

annocol1 <- annocol1[order(annocol1$stem),]

pdf("results/PCSC1/chromVAR/CREAM/KS2/Heatmap.BiasCorrectedDev.Top.pdf")
pheatmap((top_devs[,rownames(annocol1)]),
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/Tsne.stem.pdf"); print(tsne_plots) ; dev.off()

tsne_plots2 <- plotDeviationsTsne(dev, tsne_results,   annotation_name = NULL,   sample_column = "Group", shiny = FALSE)
pdf("results/PCSC1/chromVAR/CREAM/KS2/Tsne.group.pdf"); print(tsne_plots2) ; dev.off()
...
#  pdf(paste0("results/PCSC1/chromVAR/CREAM/KS2/tsne.tfplots/Tsne.",as.character(variability[f,1]),".pdf")); print(tsne_plots) ; dev.off()
#}
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/SampleCorrelation.pdf")
pheatmap((sample_cor),
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/Heatmap.BiasCorrectedDev.pdf")
pheatmap(dev.scores,
...
pdf("results/PCSC1/chromVAR/CREAM/KS2/Heatmap.ZscoreDev.pdf")
pheatmap(z.scores,
         annotation_col=annocol,clustering_method="complete",
         clustering_distance_rows="correlation",clustering_distance_cols="euclidean",
         cex=0.5,color=brewer_yes,scale="none",

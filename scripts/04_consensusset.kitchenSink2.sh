...
cat $BRAIN $ESC $HEMATDIFF $HEMATHSC $HEMATProg $GBM $PFA $HF $LSCp $LSCn $LSCb | awk 'BEGIN {FS=OFS="\t"} { print $1,$2,$3}' |\
grep -v chrM | grep -v Un | grep -v random | grep -v alt | grep -v KI27 | grep -v chrGL | grep -v chrEBV | \
sortBed -i stdin | mergeBed -i stdin | sortBed -i stdin > $OPDIR/$OPNAME/KitchenSink2.Consensus.Catalogue.narrowPeak
...
intersectBed -a $OPDIR/$OPNAME/KitchenSink2.Consensus.Catalogue.narrowPeak -b stdin -u > $OPDIR/$OPNAME/KitchenSink2.Consensus.Catalogue.Promoters.bed

awk 'BEGIN {FS=OFS="\t"} { print $1, $2-2500, $3+1000}' $GENCODETSS | grep -v chrEBV | grep -v "chrM" | grep -v "Un" | grep -v "random" | grep -v "alt" | grep -v KI27 | grep -v chrGL | \
intersectBed -a $OPDIR/$OPNAME/KitchenSink2.Consensus.Catalogue.narrowPeak -b stdin -v  > $OPDIR/$OPNAME/KitchenSink2.Consensus.Catalogue.Enhancers.bed
...
cp /mnt/work1/users/lupiengroup/People/qamraa99/common.data/ATAC-Catalogue//ATAC-ENCODE/hg38/peaks/ENCFF682RLN_peaks.narrowPeak $OPDIR/$OPNAME/temp.peaks
cp /mnt/work1/users/lupiengroup/People/qamraa99/common.data/ATAC-Catalogue//ATAC-ENCODE/hg38/peaks/ENCFF114WRE_peaks.narrowPeak $OPDIR/$OPNAME/temp.peaks
...
rename .narrowPeak.removed .narrowPeak $OPDIR/$OPNAME/temp.peaks/*.removed

Rscript $SCRIPTDIR/createbinarymat.R data/ConsensusSet/KitchenSink2/KitchenSink2.Consensus.Catalogue.narrowPeak $OPDIR/$OPNAME/temp.peaks ".narrowPeak" $OPDIR/$OPNAME/ KitchenSink2.Consensus.Catalogue.Binarymat.txt

...
binarymat <- read.table("data/ConsensusSet/KitchenSink2/KitchenSink2.Consensus.Catalogue.Binarymat.txt", header=T, sep="\t", stringsAsFactors = F,check.names = F)
rownames(binarymat) <- paste(binarymat[,1],binarymat[,2],binarymat[,3],sep="_")
prom <- read.table("data/ConsensusSet/KitchenSink2/KitchenSink2.Consensus.Catalogue.Promoters.bed", header=F, sep="\t", stringsAsFactors = F,check.names = F)
rownames(prom) <- paste(prom[,1],prom[,2],prom[,3],sep="_")

mat <- as.matrix(binarymat[rownames(prom),4:ncol(binarymat)])
...
save(rownames.obj, file=paste0("results/KitchenSink2/Cluster/KCCA.Flexclust/kccadata/KCCA.KitchenSink2.Promoter.rownames.", numk,".Rdata"))
kcca.cl <- kcca_Wouter(x=mat, k=numk, family=fam, control=ctrl,stop_percentage=0.01);
save(kcca.cl, file=paste0("results/KitchenSink2/Cluster/KCCA.Flexclust/kccadata/KCCA.KitchenSink2.Promoter.", numk,".Rdata"))

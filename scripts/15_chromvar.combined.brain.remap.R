...
rm(fragment_counts)
rm(my_counts_matrix)
save(counts_filtered, file="results/PCSC1/chromVAR/remap/brain/counts_filtered.Rdata")

#----------------------------------------------------------------
## Get motifs and what peaks contain motifs
#----------------------------------------------------------------
my_annotation_df <- fread("data/ConsensusSet/PCSC1/Combined.Brain.Consensus.Catalogue.Binarymat.with.remap.txt", header=T, sep="\t", stringsAsFactors=F, data.table=F)
anno_ix <- getAnnotations(as.matrix(my_annotation_df[,4:ncol(my_annotation_df)]), rowRanges = rowRanges(counts_filtered))
save(anno_ix, file="results/PCSC1/chromVAR/remap/brain/anno_ix.Rdata")
...
save(dev,file="results/PCSC1/chromVAR/remap/brain/Brain.extended.dev.Rdata")
z.scores = deviationScores(dev) ## deviation Z-score
dev.scores = deviations(dev) ## bias corrected deviations

write.table(z.scores, file="results/PCSC1/chromVAR/remap/brain/Dev.Zscore.txt", col.names=T, row.names=T, sep="\t", quote=F)
write.table(dev.scores, file="results/PCSC1/chromVAR/remap/brain/Deviations.txt", col.names=T, row.names=T, sep="\t", quote=F)

#----------------------------------------------------------------
## compute variablity
#----------------------------------------------------------------
variability <- computeVariability(dev)
pdf("results/PCSC1/chromVAR/remap/brain/Variability.pdf") ;
plotVariability(variability, use_plotly = FALSE) ;
dev.off()

write.table(variability, file="results/PCSC1/chromVAR/remap/brain/Variability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/remap/brain/VariabilityRanked.pdf", useDingbats = F) ;
...
pdf("results/PCSC1/chromVAR/remap/brain/Heatmap.BiasCorrectedDev.Top.pdf")
pheatmap((top_devs),
...
pdf("results/PCSC1/chromVAR/remap/brain/Tsne.stem.pdf"); print(tsne_plots) ; dev.off()

tsne_plots2 <- plotDeviationsTsne(dev, tsne_results,   annotation_name = NULL,   sample_column = "Group", shiny = FALSE)
pdf("results/PCSC1/chromVAR/remap/brain/Tsne.group.pdf"); print(tsne_plots2) ; dev.off()
...
for( f in 1:nrow(variability) ){
  ...
  pdf(paste0("results/PCSC1/chromVAR/remap/brain/tsne.tfplots/Tsne.",as.character(variability[f,1]),".pdf")); print(tsne_plots) ; dev.off()
}
...
pdf("results/PCSC1/chromVAR/remap/brain/SampleCorrelation.pdf")
pheatmap((sample_cor),
...
write.table(diff_acc, file="results/PCSC1/chromVAR/remap/brain/DiffAccessibility.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
write.table(diff_var, file="results/PCSC1/chromVAR/remap/brain/DiffVariability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/remap/brain/Heatmap.BiasCorrectedDev.pdf")
...
pdf("results/PCSC1/chromVAR/remap/brain/Heatmap.ZscoreDev.pdf")

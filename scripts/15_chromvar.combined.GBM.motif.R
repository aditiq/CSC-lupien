...
save(motif_ix,file="results/PCSC1/chromVAR/brain/motif_ix.Rdata")

#----------------------------------------------------------------
## compute deviation
#----------------------------------------------------------------
dev <- computeDeviations(object = counts_filtered, annotations = motif_ix)
save(dev,file="results/PCSC1/chromVAR/brain/Combined.Brain.extended.dev.Rdata")
z.scores = deviationScores(dev) ## deviation Z-score
dev.scores = deviations(dev) ## bias corrected deviations

write.table(z.scores, file="results/PCSC1/chromVAR/brain/Dev.Zscore.txt", col.names=T, row.names=T, sep="\t", quote=F)
write.table(dev.scores, file="results/PCSC1/chromVAR/brain/Deviations.txt", col.names=T, row.names=T, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/brain/Variability.pdf") ;
plotVariability(variability, use_plotly = FALSE) ;
dev.off()

write.table(variability, file="results/PCSC1/chromVAR/brain/Variability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/brain/VariabilityRanked.pdf", useDingbats = F) ;
...
pdf("results/PCSC1/chromVAR/brain/Heatmap.BiasCorrectedDev.Top.pdf")
pheatmap((top_devs),
...
pdf("results/PCSC1/chromVAR/brain/Tsne.stem.pdf"); print(tsne_plots) ; dev.off()

tsne_plots2 <- plotDeviationsTsne(dev, tsne_results,   annotation_name = NULL,   sample_column = "Group", shiny = FALSE)
pdf("results/PCSC1/chromVAR/brain/Tsne.group.pdf"); print(tsne_plots2) ; dev.off()
...
for( f in 1:nrow(variability) ){
  ...
  pdf(paste0("results/PCSC1/chromVAR/brain/tsne.tfplots/Tsne.",as.character(variability[f,1]),".pdf")); print(tsne_plots) ; dev.off()
}
...
pdf("results/PCSC1/chromVAR/brain/SampleCorrelation.pdf")
pheatmap((sample_cor),
...
pdf("results/PCSC1/chromVAR/brain/Heatmap.BiasCorrectedDev.pdf")
pheatmap(dev.scores,
...
pdf("results/PCSC1/chromVAR/brain/Heatmap.ZscoreDev.pdf")
pheatmap(z.scores,
         annotation_col=annocol,clustering_method="complete",
         clustering_distance_rows="correlation",clustering_distance_cols="euclidean",
         cex=0.5,color=brewer_yes,scale="none",

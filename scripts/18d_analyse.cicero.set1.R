...
analysfunc=function(name, opname, ...){
    ...
    ds <- fread(paste0("results/PCSC1/cicero/", name,".connsfull.bed"), header=T, sep="\t", stringsAsFactors=F, data.table=F)
    ...
    ds.dedup <- fread(paste0("results/PCSC1/cicero/", name,".deduped.connsfull.pairs.bed"), header=F, sep="\t", stringsAsFactors=F, data.table=F)
    ...
    ds.prom <- fread(paste0("results/PCSC1/cicero/", name,".promoters.bed"), header=F, sep="\t", stringsAsFactors=F, data.table=F) ## read in regions mapping to promoters
    ...
    write.table(ds.dedup.coaccess.anno, file=paste0("results/PCSC1/cicero/", name, ".Deduplicated.coaccess.Annotated.txt"), sep="\t", quote=F,row.names=F, col.names=T)
    ...
    write.table(summ1, file=paste0("results/PCSC1/cicero/", name, ".E-P.Anno.txt"), sep="\t", quote=F,row.names=F, col.names=T)
    summ1tab <- as.data.frame(table(summ1$peak1.anno, summ1$peak2.anno))
    colnames(summ1tab)[1:2] <- c("Peak1", "Peak2")
    write.table(summ1tab, file=paste0("results/PCSC1/cicero/", name, ".E-P.Summary.txt"), sep="\t", quote=F,row.names=F, col.names=T)
    ...
    write.table(ds.dedup.coaccess.anno.lsc17, file=paste0("results/PCSC1/cicero/", name, ".LSCp17geneshubs.txt"), sep="\t", quote=F,row.names=F, col.names=T)

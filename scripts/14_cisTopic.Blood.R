...
#runcistopic=function(ds, ...){
    binarymatds="data/ConsensusSet/PCSC1/Combined.Blood.Consensus.Catalogue.Binarymat.txt"
    ...
    cisTopicObject <- createcisTopicObject(ncounts, project.name='cisTopic_Blood')
    ...
    ## Run the model
    cisTopicObject <- runModels(cisTopicObject, topic=c(5,10,15,20,25,50,75,100), seed=987, nCores=1, burnin = 250, iterations = 500) ## build the model
    saveRDS(cisTopicObject, file="results/PCSC1/Cluster/cisTopic/cisTopicObject.Blood.v2.rds")


    cto <- readRDS("results/PCSC1/Cluster/cisTopic/cisTopicObject.Blood.v2.rds")

    pdf("results/PCSC1/Cluster/cisTopic/logLikelihoodByIter.Blood.pdf"); logLikelihoodByIter(cto) ; dev.off()
    #cto <- selectModel(cto)
    ...
    for (f in c(5,10,15,20,25,50,75,100)){
      ...
      pdf(paste0("results/PCSC1/Cluster/cisTopic/", f,".Blood.PCA.pdf")); plotCellStates(cto,dim=2,col.low = "dodgerblue4", col.mid = "floralwhite", col.high = "brown2",  method='PCA', topic_contr='Zscore', topics='all', colorBy=c('group1', 'tissue','stem')) ; dev.off()
      #pdf("chk2.pdf"); plotCellStates(cto, method='Biplot', topic_contr='Zscore', topics = c(4,7,8), colorBy=c('group1', 'tissue'))  ; dev.off()
      pdf(paste0("results/PCSC1/Cluster/cisTopic/", f,".Blood.Heatmap.pdf")); cellTopicHeatmap(cto, col.low = "dodgerblue4", col.mid = "floralwhite", col.high = "brown2", colorBy=c('group1', 'tissue','stem'))  ; dev.off()
      ...
      pdf(paste0("results/PCSC1/Cluster/cisTopic/", f,".Blood.pHeatmap.pdf"));
      pheatmap(  topic.mat,

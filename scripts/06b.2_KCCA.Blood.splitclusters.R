...
# Load binary matrix
#-----------------------------------
binarymat <- fread("data/ConsensusSet/PCSC1/Combined.Blood.Consensus.Catalogue.Binarymat.txt", header=T, sep="\t", stringsAsFactors = F,check.names = F, data.table=F)
...
for ( f in c(25,50,75,100)){
  ...
  visfunc(
  dsname=paste0(datadir,"KCCA.Blood.",f, ".Rdata"),
  rownamesds=paste0(datadir,"KCCA.Blood.rownames.",f,".Rdata"),
  mat=ksenhmat,
  opname=paste0(figdir,"K", f, ".Blood.unsuper.pdf"),
  opname2=paste0(figdir,"K", f, ".Blood.unsuper.PCA.pdf"),
  opname3=paste0(figdir,"K", f, ".Blood.txt"),
  opname4=paste0(figdir,"K", f, ".Blood.unsuper.v2.pdf")
  )
}

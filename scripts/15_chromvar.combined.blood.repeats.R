...
#----------------------------------------------------------------
my_annotation_df <- fread("data/ConsensusSet/PCSC1/Combined.Blood.Consensus.Catalogue.Binarymat.repeat.txt", header=T, sep="\t", stringsAsFactors=F, data.table=F)
...
save(dev,file="results/PCSC1/chromVAR/repeats/blood/Blood.extended.dev.Rdata")
z.scores = deviationScores(dev) ## deviation Z-score
dev.scores = deviations(dev) ## bias corrected deviations

write.table(z.scores, file="results/PCSC1/chromVAR/repeats/blood/Dev.Zscore.txt", col.names=T, row.names=T, sep="\t", quote=F)
write.table(dev.scores, file="results/PCSC1/chromVAR/repeats/blood/Deviations.txt", col.names=T, row.names=T, sep="\t", quote=F)

#----------------------------------------------------------------
## compute variablity
#----------------------------------------------------------------
variability <- computeVariability(dev)
pdf("results/PCSC1/chromVAR/repeats/blood/Variability.pdf") ;
plotVariability(variability, use_plotly = FALSE) ;
dev.off()

write.table(variability, file="results/PCSC1/chromVAR/repeats/blood/Variability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/repeats/blood/VariabilityRanked.pdf", useDingbats = F) ;
...
pdf("results/PCSC1/chromVAR/repeats/blood/Heatmap.BiasCorrectedDev.Top.pdf")
pheatmap((top_devs),
...
pdf("results/PCSC1/chromVAR/repeats/blood/Tsne.stem.pdf"); print(tsne_plots) ; dev.off()

tsne_plots2 <- plotDeviationsTsne(dev, tsne_results,   annotation_name = NULL,   sample_column = "Group", shiny = FALSE)
pdf("results/PCSC1/chromVAR/repeats/blood/Tsne.group.pdf"); print(tsne_plots2) ; dev.off()
...
for( f in 1:nrow(variability) ){
  ...
  pdf(paste0("results/PCSC1/chromVAR/repeats/blood/tsne.tfplots/Tsne.",as.character(variability[f,1]),".pdf")); print(tsne_plots) ; dev.off()
}
...
pdf("results/PCSC1/chromVAR/repeats/blood/SampleCorrelation.pdf")
pheatmap((sample_cor),
...
## Function to see whether deviations differ between groups -- "Cancer Stem" and "differentiated"
...
write.table(diff_acc, file="results/PCSC1/chromVAR/repeats/blood/DiffAccessibility.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
write.table(diff_var, file="results/PCSC1/chromVAR/repeats/blood/DiffVariability.txt", col.names=T, row.names=F, sep="\t", quote=F)
...
pdf("results/PCSC1/chromVAR/repeats/blood/Heatmap.BiasCorrectedDev.pdf")
...
pdf("results/PCSC1/chromVAR/repeats/blood/Heatmap.ZscoreDev.pdf")

...
#runcistopic=function(ds, ...){
    ...
    ## Run the model
    cisTopicObject <- runModels(cisTopicObject, topic=c(5,10,15,20,25), seed=987, nCores=1, burnin = 250, iterations = 500, returnType="allModels") ## build the model
    saveRDS(cisTopicObject, file="results/KitchenSink2/Cluster/cisTopic/cisTopicObject.5to25.rds")


    cto <- readRDS("results/KitchenSink2/Cluster/cisTopic/cisTopicObject.5to25.rds")

    pdf("results/KitchenSink2/Cluster/cisTopic/logLikelihoodByIter.5to25.pdf"); logLikelihoodByIter(cto) ; dev.off()
    #cto <- selectModel(cto)

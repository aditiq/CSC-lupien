...
binarymat <- read.table("data/ConsensusSet/KitchenSink1/KitchenSink1.Consensus.Catalogue.Binarymat.txt", header=T, sep="\t", stringsAsFactors = F,check.names = F)
rownames(binarymat) <- paste(binarymat[,1],binarymat[,2],binarymat[,3],sep="_")
prom <- read.table("data/ConsensusSet/KitchenSink1/KitchenSink1.Consensus.Catalogue.Promoters.bed", header=F, sep="\t", stringsAsFactors = F,check.names = F)
...
save(rownames.obj, file=paste0("results/KitchenSink1/Cluster/KCCA.Flexclust/kccadata/KCCA.KitchenSink1.Promoter.rownames.", numk,".Rdata"))
kcca.cl <- kcca_Wouter(x=mat, k=numk, family=fam, control=ctrl,stop_percentage=0.0001);
save(kcca.cl, file=paste0("results/KitchenSink1/Cluster/KCCA.Flexclust/kccadata/KCCA.KitchenSink1.Promoter.", numk,".Rdata"))
